      ...
      <CardContent>
        ...
        >
          {sections.map((section) => {
            const { positiveSum, negativeSum, netTotal } =
              calculateSectionSummary(section.transactions);
                    ...
                    {isOpen && (
                      <div className="hidden lg:flex space-x-4 ml-4">
                        ...
                        <span className="text-red-600">
                          -{formatCurrency(Math.abs(negativeSum))}
                        </span>
                        <span
                          className={`font-bold ${netTotal < 0 ? "text-red-600" : "text-green-600"}`}
                        >
                          {netTotal < 0 ? "-" : "+"}
                        ...
                        <TableRow className="border-b-4 border-gray-600">
                          {showId && (
                            <TableHead
                              ...
                              onClick={() =>
                                handleSort("id", section.setSortConfig)
                              }
                            >
                              Id{" "}
                              {section.sortConfig?.key === "id" &&
                                (section.sortConfig.direction === "asc"
                                  ? "↑"
                                  : "↓")}
                            </TableHead>
                          )}
                          {showDate && (
                            <TableHead
                              ...
                              onClick={() =>
                                handleSort("Date", section.setSortConfig)
                              }
                            >
                              Date{" "}
                              {section.sortConfig?.key === "Date" &&
                                (section.sortConfig.direction === "asc"
                                  ? "↑"
                                  : "↓")}
                            </TableHead>
                          ...
                          <TableHead
                            ...
                            onClick={() =>
                              handleSort("Description", section.setSortConfig)
                            }
                          >
                            Description{" "}
                            {section.sortConfig?.key === "Description" &&
                              (section.sortConfig.direction === "asc"
                                ? "↑"
                                : "↓")}
                          </TableHead>
                          {showComments && (
                            <TableHead
                              className="font-bold cursor-pointer w-48"
                              onClick={() =>
                                handleSort("Comment", section.setSortConfig)
                              }
                            >
                              Comment{" "}
                              {section.sortConfig?.key === "Comment" &&
                                (section.sortConfig.direction === "asc"
                                  ? "↑"
                                  : "↓")}
                            </TableHead>
                          ...
                          <TableHead
                            ...
                            onClick={() =>
                              handleSort("Amount", section.setSortConfig)
                            }
                          >
                            Amount{" "}
                            {section.sortConfig?.key === "Amount" &&
                              (section.sortConfig.direction === "asc"
                                ? "↑"
                                : "↓")}
                          </TableHead>
