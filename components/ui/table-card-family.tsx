                    ...
                    <Table style={{ fontFamily: 'Menlo, Monaco, Consolas, Courier New, monospace' }}>
                      <TableHeader>
                        <TableRow className="border-b-4 border-gray-600">
                          ...
                          <TableHead className="font-bold cursor-pointer w-32" onClick={() => handleSort('Date', section.setSortConfig)}>
                            Date {section.sortConfig?.key === 'Date' && (section.sortConfig.direction === 'asc' ? '↑' : '↓')}
                          </TableHead>
                          <TableHead className="font-bold cursor-pointer w-48" onClick={() => handleSort('Description', section.setSortConfig)}>
                          ...
                          <TableHead className="font-bold cursor-pointer w-24 text-right" onClick={() => handleSort('Amount', section.setSortConfig)}>
                      ...
                      <TableBody>
                        {sortTransactions(section.transactions, section.sortConfig).map((transaction) => (
                          <TableRow key={transaction.id} className="text-xs">
                            ...
                            <TableCell className="w-32">{transaction.Date ? new Date(transaction.Date).toLocaleDateString() : 'N/A'}</TableCell>
                            ...
                            <TableCell className={`w-24 text-right ${transaction.Amount && transaction.Amount < 0 ? 'text-red-600' : 'text-green-600'}`}>
                              {transaction.Amount && transaction.Amount < 0 ? '-' : '+'}
                              {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Math.abs(transaction.Amount ?? 0))}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
